...
int grammer_check(char* a0p, char* a1p, char* a2p, char* a3p, char* a4p, char* a5p, int pc, int* b0, int* b1, int* b2, int* b3) {
  ...
  else if (strcmp(a1p, "010010") == 0) { //ftor
    b0[pc] = 55;
    b1[pc] = strtol(&a1p[0], NULL, 2);
    b2[pc] = strtol(&a2p[0], NULL, 2);
    return 0;
  ...
  else if (strcmp(a2p, "010111") == 0) { //bff
    b0[pc] = 49;
    b1[pc] = (short)strtol(&a345[0], NULL, 2);
    return 0;
  }
...
int main(int argc, char* argv[]) {
    ...
    // for (i = 0; i < 10000; i++) {
    //   if (i < 2)
    //     fgets(gomi[i], 200, fd2);
    //   else if (fgets(f_im_sub[i - 2], 200, fd2) == NULL)
    //     break;
    //   else
    //     f_im[i - 2] = strtol(&f_im_sub[i - 2], NULL, 16);
    // }
